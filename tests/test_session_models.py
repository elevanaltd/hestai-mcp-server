...
class TestSessionContextModel:
    ...
    def test_valid_session_context(self):
        ...
        session = SessionContextModel(session_id="test-session-123", project_root=Path("/tmp/test-project"))
    ...
    def test_session_id_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            SessionContextModel(session_id="", project_root=Path("/tmp/test"))
    ...
    def test_project_root_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            SessionContextModel(session_id="test-session", project_root=Path("relative/path"))
    ...
    def test_to_dict_conversion(self):
        ...
        session = SessionContextModel(session_id="test-session", project_root=Path("/tmp/test-project"))

        result = session.to_dict()
        expected = {"session_id": "test-session", "project_root": "/tmp/test-project"}
...
class TestToolExecutionContext:
    ...
    def test_valid_execution_context(self):
        ...
        session = SessionContextModel(session_id="exec-session", project_root=Path("/tmp/exec-project"))

        context = ToolExecutionContext(session=session, remaining_tokens=5000, model_name="gpt-4")
    ...
    def test_get_project_root(self):
        ...
        # RED: This method should exist but doesn't yet
        session = SessionContextModel(session_id="test", project_root=Path("/tmp/test"))
    ...
    def test_get_session_id(self):
        ...
        # RED: This method should exist but doesn't yet
        session = SessionContextModel(session_id="test-id", project_root=Path("/tmp/test"))
    ...
    def test_optional_fields(self):
        ...
        session = SessionContextModel(session_id="minimal", project_root=Path("/tmp/minimal"))
...
class TestSecurityValidation:
    ...
    def test_path_traversal_prevention(self):
        ...
        dangerous_paths = ["../../../etc/passwd", "../../sensitive", "/tmp/../../../etc"]

        for dangerous_path in dangerous_paths:
            with pytest.raises(ValidationError):
                SessionContextModel(session_id="dangerous", project_root=Path(dangerous_path))
    ...
    def test_fail_fast_validation(self):
        ...
        with pytest.raises(ValidationError):
            SessionContextModel(session_id=None, project_root=Path("/tmp/test"))  # Invalid

        with pytest.raises(ValidationError):
            SessionContextModel(session_id="valid", project_root=None)  # Invalid
