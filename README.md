...
but gets perspectives from the best AI for each subtask. With tools like [`planner`](#4-planner---interactive-step-by-step-planning) for
breaking down complex projects, [`analyze`](#7-analyze---smart-file-analysis) for understanding codebases,
and [`debug`](#6-debug---expert-debugging-assistant) for solving complex problems, Claude can switch between different tools _and_ models mid-conversation,
...
- **Tools Reference**
  ...
  - [`secaudit`](#9-secaudit---comprehensive-security-audit) - Security audit with OWASP analysis
...
- **Guided workflows** - Developer-centric processes that enforce systematic investigation, preventing rushed analysis by ensuring Claude examines code thoroughly at each phase ([`debug`](#6-debug---expert-debugging-assistant), [`analyze`](#7-analyze---smart-file-analysis))
...
- **Get multiple expert opinions** - Have different AI models debate your ideas (some supporting, some critical) to help you make better decisions ([`consensus`](#5-consensus---multi-model-perspective-gathering))
...
```json
{
  "mcpServers": {
    "hestai": {
      ...
      "args": [
        "-c",
        "exec $(which uvx || echo uvx) --from git+https://github.com/elevanaltd/hestai-mcp-server.git hestai-mcp-server"
      ],
      "env": {
...
6. [`debug`](docs/tools/debug.md) - Systematic investigation and debugging
7. [`analyze`](docs/tools/analyze.md) - General-purpose file and code analysis
8. [`tracer`](docs/tools/tracer.md) - Static code analysis prompt generator for call-flow mapping
9. [`secaudit`](docs/tools/secaudit.md) - Comprehensive security audit with OWASP Top 10 analysis
10. [`listmodels`](docs/tools/listmodels.md) - Display all available AI models organized by provider
11. [`version`](docs/tools/version.md) - Get server version and configuration
...
[![Star History Chart](https://api.star-history.com/svg?repos=elevanaltd/hestai-mcp-server&type=Date)](https://www.star-history.com/#elevanaltd/hestai-mcp-server&Date)
