"""Centralized environment variable access for HestAI MCP Server.

Ported from Zen-MCP-Server upstream (Phase 0.5 STEP 3a)
...
def _compute_force_override(values: Mapping[str, str | None]) -> bool:
    raw = (values.get("HESTAI_MCP_FORCE_ENV_OVERRIDE") or "false").strip().lower()
    return raw == "true"
...
def env_override_enabled() -> bool:
    """Return True when HESTAI_MCP_FORCE_ENV_OVERRIDE is enabled via the .env file."""

    return _FORCE_ENV_OVERRIDE


def get_env(key: str, default: str | None = None) -> str | None:
    """Retrieve environment variables respecting HESTAI_MCP_FORCE_ENV_OVERRIDE."""
