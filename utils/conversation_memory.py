...
Ported from Zen-MCP-Server upstream (Phase 0.5 STEP 3b)
...
conversations between Claude and downstream models by storing conversation
...
- LLM sees: "--- Turn 3 (Claude) ---", "--- Turn 4 (Model) ---", "--- Turn 5 (Claude) ---"
...
class ConversationTurn(BaseModel):
    ...
    Attributes:
        role: "user" (Claude request) or "assistant" (model response)
...
def build_conversation_history(context: ThreadContext, model_context=None, read_files_func=None) -> tuple[str, int]:
    ...
    # The LLM will see: "--- Turn 1 (Claude) ---" followed by "--- Turn 2 (Model) ---" etc.
