...
from .security_config import is_dangerous_path, EXCLUDED_DIRS
...
class FileContextProcessor:
        ...
        for file_path in file_paths:
            try:
                ...
                if is_dangerous_path(path):
                    result["files"].append({
                        "path": file_path,
                        "error": "Access denied - security restriction"
                    })
                    continue
        ...
        try:
            ...
            if include_configs:
                config_files = [
                    ...
                    "tsconfig.json", "jest.config.js", "pytest.ini", ".coveragerc"
                ]
    ...
    def get_architectural_context(self, project_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            config_patterns = [
                "package.json", "pyproject.toml", "setup.py", "requirements*.txt",
                "Cargo.toml", "go.mod", "pom.xml", "build.gradle", "*.config.js",
            ...
            entry_patterns = [
                ...
                "Main.java", "server.py", "cli.py", "run.py"
            ]
            ...
            if (root / "requirements.txt").exists():
                content = read_file_content(str(root / "requirements.txt"))
                if content:
                ...
                content = read_file_content(str(root / "package.json"))
                if content:
                    try:
    ...
    def get_test_context(self, project_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            test_config_patterns = [
                "pytest.ini", "pytest.cfg", "setup.cfg", "tox.ini",
                "jest.config.js", "karma.conf.js", "mocha.opts",
                ".coveragerc", "coverage.json", "phpunit.xml"
            ]
