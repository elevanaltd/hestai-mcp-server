...
Write-ColorText "=== Deploying HestAI MCP Server ===" -Color Green
...
function Wait-ForHealth {
    ...
    while ($attempt -le $MaxAttempts) {
        try {
            # Get container ID for hestai-mcp service
            $containerId = docker-compose ps -q hestai-mcp
...
# Start the services
Write-ColorText "Starting HestAI MCP Server..." -Color Green
try {
    docker-compose up -d
    if ($LASTEXITCODE -ne 0) {
...
Write-ColorText "✓ HestAI MCP Server deployed successfully" -Color Green
...
Write-ColorText "docker-compose logs -f hestai-mcp" -Color Green
...
Write-ColorText "docker-compose restart hestai-mcp" -Color Green
