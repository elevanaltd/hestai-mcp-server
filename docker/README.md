...
docker build -t hestai-mcp-server:latest .
...
docker images hestai-mcp-server

# Test container interactively
docker run --rm -it --env-file .env hestai-mcp-server:latest bash

# Check environment variables
docker run --rm --env-file .env hestai-mcp-server:latest env | grep API

# Test with minimal configuration
docker run --rm -i -e GEMINI_API_KEY="test" hestai-mcp-server:latest python server.py
```
...
echo '{"jsonrpc": "2.0", "method": "ping"}' | docker run --rm -i --env-file .env hestai-mcp-server:latest python server.py
...
docker run --rm --env-file .env hestai-mcp-server:latest python -c "import os; print('GEMINI_API_KEY:', bool(os.getenv('GEMINI_API_KEY')))"

# Test API connectivity
docker run --rm --env-file .env hestai-mcp-server:latest python /usr/local/bin/healthcheck.py
```
...
docker run --rm -i --memory="512m" --env-file .env hestai-mcp-server:latest

# Monitor Docker logs
docker run --rm -i --env-file .env hestai-mcp-server:latest 2>&1 | tee docker.log
```
  ...
  "servers": {
    "zen-docker": {
      ...
      "args": [
        ...
        "-i",
        "--env-file",
        "C:/Users/YourName/path/to/hestai-mcp-server/.env",
        "-v",
        "C:/Users/YourName/path/to/hestai-mcp-server/logs:/app/logs",
        "hestai-mcp-server:latest"
      ]
    }
  }
...
docker run --rm hestai-mcp-server:latest python --version

# Test health check
docker run --rm -e GEMINI_API_KEY="test" hestai-mcp-server:latest python /usr/local/bin/healthcheck.py
```
...
echo '{"jsonrpc": "2.0", "method": "initialize", "params": {}}' | \
  docker run --rm -i --env-file .env hestai-mcp-server:latest python server.py
```
