...
cross-platform compatibility issues in the hestai-mcp-server project.
...
4. SHELL SCRIPTS WINDOWS COMPATIBILITY:
    ...
    - Solution: Added detection for .hestai_venv/Scripts/ paths

5. SHELL SCRIPTS PYTHON AND TOOL DETECTION:
    - Python and tool executables not detected on Windows
    - Solution: Added detection for .hestai_venv/Scripts/*.exe paths
...
class CrossPlatformPatcher:
    ...
    def patch_shell_venv_detection(self, content: str) -> tuple[str, bool]:
        ...
        if 'elif [[ -f ".hestai_venv/Scripts/activate" ]]; then' in content:
            return content, False

        # Patch run_integration_tests.sh
        old_venv_check = """# Activate virtual environment
if [[ -f ".hestai_venv/bin/activate" ]]; then
...
if [[ -f ".hestai_venv/bin/activate" ]]; then
...
elif [[ -f ".hestai_venv/Scripts/activate" ]]; then
    source .hestai_venv/Scripts/activate
    ...
    def patch_shell_python_detection(self, content: str) -> tuple[str, bool]:
        ...
        # Check if already patched
        if 'elif [[ -f ".hestai_venv/Scripts/python.exe" ]]; then' in content:
            return content, False
...
if [[ -f ".hestai_venv/bin/python" ]]; then
...
elif [[ -f ".hestai_venv/Scripts/python.exe" ]]; then
    PYTHON_CMD=".hestai_venv/Scripts/python.exe"
    PIP_CMD=".hestai_venv/Scripts/pip.exe"
    ...
    def patch_shell_tool_paths(self, content: str) -> tuple[str, bool]:
        ...
        # Check if already patched
        if 'elif [[ -f ".hestai_venv/Scripts/ruff.exe" ]]; then' in content:
            return content, False
...
if [[ -f ".hestai_venv/bin/ruff" ]]; then
    ...
    ISORT=".hestai_venv/bin/isort"
    PYTEST=".hestai_venv/bin/pytest"
elif [[ -f ".hestai_venv/Scripts/ruff.exe" ]]; then
    RUFF=".hestai_venv/Scripts/ruff.exe"
    BLACK=".hestai_venv/Scripts/black.exe"
    ISORT=".hestai_venv/Scripts/isort.exe"
    PYTEST=".hestai_venv/Scripts/pytest.exe"
else
    RUFF="ruff"
    BLACK="black"
    ISORT="isort"
    ...
    def patch_base_test_python_path(self, content: str) -> tuple[str, bool]:
        ...
        \"\"\"Get the Python path for the virtual environment\"\"\"
        current_dir = os.getcwd()
        venv_python = os.path.join(current_dir, ".hestai_venv", "bin", "python")
    ...
    def validate_patches(self, files: dict[str, Path]) -> list[str]:
        ...
        if "run_integration_tests_sh" in files:
            run_integration_content = self.read_file(files["run_integration_tests_sh"])
            if 'elif [[ -f ".hestai_venv/Scripts/activate" ]]; then' not in run_integration_content:
                errors.append("Windows venv detection missing in run_integration_tests.sh")

        if "code_quality_checks_sh" in files:
            code_quality_content = self.read_file(files["code_quality_checks_sh"])
            if 'elif [[ -f ".hestai_venv/Scripts/python.exe" ]]; then' not in code_quality_content:
                errors.append("Windows Python detection missing in code_quality_checks.sh")
            if 'elif [[ -f ".hestai_venv/Scripts/ruff.exe" ]]; then' not in code_quality_content:
        ...
        modifications = [
            ...
            (
                ...
                [
                    ...
                    "Support .hestai_venv/Scripts/activate path",
                ],
            ),
            (
                ...
                [
                    ...
                    "Support .hestai_venv/Scripts/*.exe tool paths",
                ],
            ),
            (
