...
class TestCriticalEngineerEnhanced:
    ...
    def mock_llm_provider(self):
        ...
        provider = AsyncMock()
        provider.chat_completion = AsyncMock(return_value="VIABILITY_ASSESSMENT: VIABLE\n\nValidation complete.")
        return provider
...
"""
    ...
    async def test_request_with_files(self, tool, temp_project):
        ...
        request = CriticalEngineerRequest(
            ...
            files=[str(temp_project / "src" / "api.py"), str(temp_project / "src" / "database.py")],
        )
    ...
    async def test_request_with_tree_option(self, tool, temp_project):
        ...
        request = CriticalEngineerRequest(prompt="Validate overall system architecture", include_tree=True, max_depth=3)
    ...
    async def test_request_backwards_compatible(self, tool):
        ...
        request = CriticalEngineerRequest(prompt="Validate this architecture decision")
    ...
    async def test_prepare_prompt_with_files(self, tool, temp_project, mock_llm_provider):
        ...
        request = CriticalEngineerRequest(
            prompt="Validate the API security", files=[str(temp_project / "src" / "api.py")]
        )
        ...
        with patch("tools.critical_engineer.FileContextProcessor") as mock_processor:
            ...
            mock_instance.get_relevant_files.return_value = {
                "files": [{"path": "src/api.py", "content": "class APIHandler:..."}],
                "total_tokens": 50,
            }
    ...
    async def test_prepare_prompt_with_tree(self, tool, temp_project):
        ...
        request = CriticalEngineerRequest(prompt="Validate system architecture", include_tree=True, max_depth=2)
    ...
    async def test_validation_with_file_context(self, tool):
        ...
        request = CriticalEngineerRequest(prompt="Validate SQL injection vulnerabilities", files=["src/database.py"])
        ...
        with patch("tools.critical_engineer.FileContextProcessor") as mock_processor:
            ...
            mock_instance.get_relevant_files.return_value = {
                "files": [
                    {"path": "src/database.py", "content": "def query(self, sql): return self.conn.execute(sql)"}
                ],
                "total_tokens": 30,
            }
            ...
            assert "src/database.py" in prompt
    ...
    async def test_handle_missing_files_gracefully(self, tool):
        ...
        request = CriticalEngineerRequest(prompt="Validate architecture", files=["/nonexistent/file.py"])

        with patch("tools.critical_engineer.FileContextProcessor") as mock_processor:
            mock_instance = MagicMock()
            mock_processor.return_value = mock_instance
            mock_instance.get_relevant_files.return_value = {
                "files": [{"path": "/nonexistent/file.py", "error": "File not found"}],
                "total_tokens": 0,
            }
    ...
    async def test_security_validation_paths(self, tool):
        ...
        for path in dangerous_paths:
            request = CriticalEngineerRequest(prompt="Validate", files=[path])
    ...
    async def test_auto_find_related_files(self, tool, temp_project):
        ...
        with patch("tools.critical_engineer.FileContextProcessor") as mock_processor:
            mock_instance = MagicMock()
            mock_processor.return_value = mock_instance
            mock_instance.find_related_files.return_value = [str(temp_project / "tests" / "test_api.py")]
            mock_instance.get_relevant_files.return_value = {
                "files": [{"path": "src/api.py", "content": "..."}, {"path": "tests/test_api.py", "content": "..."}],
                "total_tokens": 100,
            }
